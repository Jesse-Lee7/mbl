	...
	float * p_img32f_sub_bk, float * p_img32_sub_label, unsigned char * p_img_sub, map <int, float *> & density_map_sub)
{
	if (input_Parameter.resample != 1)
	{
		...
		if (input_Parameter.Select_modal == 0 && !density_map_sub.empty())
		{
			for (int i = 0; i<8; i++)
			{
		...
		if (input_Parameter.Select_modal == 0 && !density_map_sub_resample.empty())
		{
			for (int i = 0; i < 9; i++)
			{
...
//	clock_t stps_mBrainAligner;
//	stps_mBrainAligner = clock();
...
//	printf("\t>>stps_mBrainAligner time consume %.2f s\n", (float)(clock() - stps_mBrainAligner) / CLOCKS_PER_SEC);
	return true;
}
...
//					printf("mark_q_1 ");
		...
		//warp subject image based on updated landmark position计算四次
		...
		if ((iter) % int(input_Parameter.fre_save) == 0)
		{
			...
			//clock_t stps_imgwarp_smallmemory;
			//stps_imgwarp_smallmemory = clock();
